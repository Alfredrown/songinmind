...
import { GoogleGenerativeAI } from "@google/generative-ai"; // Ensure this library supports your use case
  ...
  const Searcher = (token) => {
    ...
    const generateText = async () => {
      ...
      try {
        ...
        const genAI = new GoogleGenerativeAI(process.env.GEMINIKEY); // Replace with your Gemini API key
        const model = genAI.getGenerativeModel({ model: "gemini-pro" });
    
        // Analyze the user's input to determine the mood/emotion
      ...
      Text: "${text}"
       ...
       // Split the suggested genres into an array
       const genresList = suggestedGenres.split(",").map((genre) => genre.trim());
   
       // Match the suggested genres with the songdata hashmap
       let matchedGenre = null;
       for (const genre of genresList) {
         const foundGenre = songdata.find((song) =>
           song.name.toLowerCase().includes(genre)
         );
         if (foundGenre) {
           matchedGenre = foundGenre.name;
           break; // Stop at the first match
         }
       }
   
       if (!matchedGenre) {
         setOutput("No matching genre found. Please try again.");
         return;
       }
   
       // Fetch a random song from the matched genre using Spotify API
       const spotifyResponse = await axios.get(
         `https://api.spotify.com/v1/search?q=genre:${matchedGenre}&type=track&limit=50`,
         {
           headers: {
       ...
       if (tracks.length === 0) {
         setOutput("No songs found for this genre. Please try again.");
         return;
       }
